...
- **高信頼なライフサイクル管理と安全な停止シーケンス**: 無人検知や定時停止によるコスト削減に加え、Lambdaが主導する「安全な停止シーケンス」を実装。RCONセーブ、S3同期のカタログ更新、OSレベルのハングアップを防ぐ **Lazy Unmount**、そしてEC2停止を適切な順序で自動制御し、データ破損リスクの低減を図っています（最終的な同期確認は `/status` や `/restore list` での反映確認を推奨）。
